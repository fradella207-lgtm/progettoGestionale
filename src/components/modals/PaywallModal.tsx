...
export const PaywallModal: React.FC<PaywallModalProps> = ({
  ...
  const handleOpenStripeLink = (planId: 'annual' | 'lifetime') => {
    const stripeUrl = STRIPE_PAYMENT_URLS[planId] || STRIPE_PAYMENT_URLS.lifetime;
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-2.5 sm:p-5 overflow-y-auto animate-in fade-in duration-200">
      <div 
        className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-lg shadow-xl border border-slate-200 overflow-hidden flex flex-col max-h-[94dvh] sm:max-h-[90vh] my-auto font-['Plus_Jakarta_Sans',sans-serif] animate-in zoom-in-98 duration-150"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* HEADER CHIARO, LEGGERO ED ELEGANTE */}
        <div className="shrink-0 p-4 sm:p-6 border-b border-slate-100 relative bg-white">
          <button 
            type="button"
            id="btn-close-paywall-modal"
            onClick={onClose}
            aria-label="Chiudi finestra abbonamento"
            className="absolute top-3.5 right-3.5 sm:top-5 sm:right-5 p-2 rounded-full text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>

          <div className="flex items-center gap-2 mb-2 sm:mb-3">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-slate-900 text-white text-[10px] sm:text-[11px] font-bold tracking-wide uppercase">
          ...
          {featureDetail && (
            <div className="mt-2.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 flex items-center gap-2">
        ...
        <div className="p-3.5 sm:p-6 flex-1 flex flex-col gap-3.5 sm:gap-4 overflow-y-auto">
        ...
        <div className="shrink-0 p-3.5 sm:p-5 bg-white border-t border-slate-100 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center justify-between sm:block min-w-0">
            ...
            <button
              type="button"
              onClick={onClose}
              className="sm:hidden px-3 py-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800 rounded-lg transition-colors cursor-pointer"
            >
              Chiudi
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 sm:shrink-0 w-full sm:w-auto">
            <button
              type="button"
              onClick={onClose}
              className="hidden sm:inline-flex px-3 py-2 text-xs font-semibold text-slate-500 hover:text-slate-800 rounded-xl transition-colors cursor-pointer"
            >
              Chiudi
            </button>
            ...
            <button
              ...
              className="w-full sm:w-auto px-5 py-3 sm:py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-97 text-white text-xs sm:text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs min-h-[44px]"
            >
