...
export const GarageHome: React.FC<GarageHomeProps> = ({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-6 font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <section className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-black uppercase tracking-wider text-slate-500">
                Garage Virtuale • {vehicles.length} {vehicles.length === 1 ? 'Veicolo Registrato' : 'Veicoli Registrati'}
              </span>
            </div>
          ...
          <button
            ...
            className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-xs sm:text-sm font-bold px-4 py-2.5 rounded-2xl shadow-xs hover:shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
        ...
        {vehicles.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-white border border-slate-200/80 p-3.5 sm:p-4 rounded-3xl shadow-2xs">
            <div className="flex items-center gap-3 p-2">
              <div className="w-10 h-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                <Car className="w-5 h-5" />
              </div>
              <div className="min-w-0">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block truncate">Auto Attive</span>
            ...
            <div className="flex items-center gap-3 p-2">
              ...
              <div className="min-w-0">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block truncate">Spese Totali</span>
            ...
            <div className="flex items-center gap-3 p-2">
              ...
              <div className="min-w-0">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block truncate">Registrazioni</span>
                <span className="text-sm sm:text-base font-black text-slate-900 truncate">
        ...
        <section className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Cerca per marca, modello, targa o motore..."
              className="w-full pl-9 pr-3.5 py-2 rounded-2xl bg-white border border-slate-200 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-2xs"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            {[
              ...
              { id: 'electric_hybrid', label: 'Ibrida / Elettrica' },
      ...
      {/* 3. VEHICLES GRID */}
      {vehicles.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-200/80 p-12 text-center flex flex-col items-center justify-center gap-4 shadow-xs">
          <div className="w-16 h-16 rounded-3xl bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center shadow-xs">
            <Car className="w-8 h-8 stroke-[1.5]" />
          </div>
          <div className="max-w-md">
            <h3 className="text-lg font-bold text-slate-900">Il tuo garage è vuoto</h3>
          ...
          <button
            onClick={onOpenAddCar}
            className="bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-xs sm:text-sm font-bold px-5 py-2.5 rounded-2xl shadow-xs transition-all flex items-center gap-2 cursor-pointer"
          >
            <Plus className="w-4 h-4" />
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-8 text-center flex flex-col items-center justify-center gap-3">
          ...
          {filteredVehicles.map((car) => {
            ...
            return (
              <div 
                ...
                className="bg-white rounded-3xl border border-slate-200/80 overflow-hidden shadow-2xs hover:shadow-lg hover:border-blue-400/50 hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between cursor-pointer group"
              >
                <div>
                  {/* PHOTO CONTAINER */}
                  <div className="w-full h-48 bg-slate-900 relative overflow-hidden flex items-center justify-center border-b border-slate-100">
                    {car.photoUrl ? (
                      <img 
                        src={car.photoUrl} 
                      ...
                      <div className="flex flex-col items-center text-slate-400 gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-300">{car.brand}</span>
                      </div>
                    )}
                    ...
                    {/* Fuel Badge */}
                    <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-md text-slate-900 text-[10px] font-extrabold px-2.5 py-1 rounded-xl border border-white/40 shadow-xs flex items-center gap-1.5">
                      {isElectric ? (
                    ...
                    {/* Manual / Specs Badge */}
                    {hasManual && (
                      <div className="absolute bottom-3 left-3 bg-slate-900/85 backdrop-blur-md text-white text-[9px] font-bold px-2 py-0.5 rounded-lg border border-white/20 flex items-center gap-1">
                        <BookOpen className="w-2.5 h-2.5 text-blue-400" />
                        <span>Manuale Indicizzato</span>
                      </div>
                    )}
                    ...
                    <div className="absolute top-3 right-3 flex items-center gap-1.5 opacity-90 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="bg-white/95 backdrop-blur-md hover:bg-white text-slate-600 hover:text-blue-600 p-2 rounded-xl border border-white/40 shadow-xs transition-all active:scale-90"
                      ...
                      <button
                        ...
                        className="bg-white/95 backdrop-blur-md hover:bg-white text-slate-600 hover:text-red-600 p-2 rounded-xl border border-white/40 shadow-xs transition-all active:scale-90"
                  ...
                  <div className="p-4 sm:p-5 flex flex-col gap-3.5">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="bg-white border-2 border-slate-300 rounded-lg px-2 py-0.5 shadow-2xs inline-flex items-center gap-1.5 shrink-0">
                        <span className="bg-blue-600 text-white text-[8px] font-black px-1 py-0.2 rounded-[2px]">IT</span>
                        <span className="text-xs font-black font-mono tracking-[1.5px] text-slate-900 uppercase">{car.plate}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-100 text-center">
                      <div>
                        <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Chilometri</span>
                        <span className="text-xs font-black text-slate-900">
                          {currentKm.toLocaleString('it-IT')} km
                        </span>
                      </div>
                      <div>
                        <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Serbatoio</span>
                        <span className="text-xs font-black text-slate-900">
                          {car.tankCapacity > 0 ? `${car.tankCapacity} ${fuelUnit}` : '-'}
                        </span>
                      </div>
                      <div>
                        <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Spesa Totale</span>
                ...
                {/* CARD FOOTER */}
                <div className="px-4 py-3 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between text-xs text-blue-600 font-bold">
                  <span className="text-slate-500 font-medium text-[11px]">
                    {(car.refuels?.length || 0)} rifornimenti • {(car.maintenances?.length || 0)} interventi
                  </span>
                  <span className="flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    Visualizza Scheda <ChevronRight className="w-4 h-4" />
                  </span>
                </div>
